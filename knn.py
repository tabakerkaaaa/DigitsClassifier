...
class KNN:
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.zeros((num_test, num_train), np.float32)
        kek = np.tile(X, (1, num_train)).reshape((num_test, num_train, -1))
        dists = np.sum(np.abs(kek - self.train_X), axis = 2)
        return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            k_min = dists[i][idx[self.k - 1]]
            binary_k_mins = dists[i] <= k_min
            k_mins = self.train_y[binary_k_mins]
            first_class = np.sum(k_mins)
            second_class = self.k - first_class
            pred[i] = first_class > second_class
        return pred
